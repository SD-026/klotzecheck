...
const App = () => {
  ...
  const fetchProxiedImage = async (url) => {
    const proxyUrl = `https://klotzecheck-2.onrender.com/proxy?url=${encodeURIComponent(url)}`;
  ...
  const generateVideo = async () => {
    ...
    try {
      ...
      const response = await axios.post(
        'https://klotzecheck-2.onrender.com/generate-video',
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const response = await axios.post(
        "https://klotzecheck-2.onrender.com/extract",
        { url },
        {
          headers: {
